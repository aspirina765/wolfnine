...
const CrawlerProduct = () => {
  ...
  const getListData = async () => {
    await crawlerProductService
      ...
      .then((res) => {
        ...
        console.log('🚀 ~ file: CrawlerProduct.jsx ~ line 102 ~ .then ~ res.data.data', res.data.data);
      })
      .catch((err) => {
        console.log('🚀 ~ file: CrawlerProduct.jsx ~ line 105 ~ getListData ~ err', err);
      });
  };
  ...
  const handleDeleteItem = async (item) => {
    await crawlerProductService.deleteById(item.id).then((res) => {
      getListData(0);
    });
  };

  const handleDeleteByIds = async () => {
    await crawlerProductService.deleteByIds(selected).then((res) => {
      getListData();
    });
  };
                    ...
                    <TableRow hover key={index} tabIndex={-1} role="checkbox" selected={false} aria-checked={false}>
                      ...
                      <TableCell component="th" scope="row">
                        <Stack direction="row" alignItems="center" spacing={2}>
                          <img src={config.attributes?.[1]?.value} width={120} />
                        </Stack>
                      </TableCell>
                      <TableCell component="th" scope="row">
                        <Stack direction="row" alignItems="center" spacing={2}>
                          <Typography variant="subtitle2" noWrap>
                            {config.attributes?.[0]?.value}
                          </Typography>
                        </Stack>
                      </TableCell>
                      <TableCell align="left">{config.crawlCategory?.link}</TableCell>
                      ...
                      <TableCell align="right">
                        <MoreMenuCustom
                          ...
                          item={config}
                          editLink={generateRouteWithParam(ROUTES.EDIT_CRAWLER_PRODUCT, ':id', config.id)}
                        />
                      </TableCell>
                    </TableRow>
                  ))}
